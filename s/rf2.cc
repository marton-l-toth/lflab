...
struct SparseRF; typedef int (comb_fun_dt)(SparseRF*, int, double*, double*, int),
      			     (*comb_fun_t)(SparseRF*, int, double*, double*, int);

struct SparseRF : BoxInst_BU {
	static scf_t sc_ini, sc_f1;   static dtorf_t dtf;
	static comb_fun_dt c00, SPFUN_LS;
	void ini(double ** ap);
	int m_nx, m_ny, m_siz, m_t, *m_xt, *m_yt;
...
void SparseRF::ini(double** p) {
	...
	int dmpf, nxy = m_arg, n_xy1 = 0, siz;
	...
	char *blk = (char*)malloc(12*n_xy1);  double *xc = m_xc = (double*)blk, *yc = xc + n_xy1;
	m_px = 0; set_dtf(&dtf);
	...
	double *px = m_px = (double*)malloc(16*siz), *py = m_py = px + siz; m_t = 0; m_siz = siz;
	for (int i=siz-tlim; i<siz; i++) px[i] = 0.0;
	for (int i=siz-tlim; i<siz; i++) py[i] = 0.0;
	m_fun = ftab[5*min_i(m_nx, 4)+min_i(m_ny,4)];
}

void SparseRF::dtf(BoxInst *p) { SparseRF *q=(SparseRF*)p; free(q->m_px); free(q->m_xc); }
BX_SCALC(SparseRF::sc_ini) { SCALC_BXI(SparseRF); bxi->ini(inb+1); CALC_FW(sc_f1); }
...
struct NAccFilt : BoxInst_B1 { static scf_t sc_ini, sc_f1, sc_fn; double m_x; };

BX_SCALC(NAccFilt::sc_ini) {
	SCALC_BXI(NAccFilt); int m = bxi->m_arg = ivlim((int)lround(inb[3][0]),0,256);
	switch(m){ case 0:							 CALC_FW(sc_cp0);
		   case 1:  bxi->m_x=0.0;					 CALC_FW(sc_f1);
		   default: bxi->m_x=0.0; m=8*m-1; memset(bxi->alloc0(m), 0, m); CALC_FW(sc_fn); }}

BX_SCALC(NAccFilt::sc_f1) { SCALC_BXI(NAccFilt); int f1 = inflg&1, md = (int)lround(inb[1][0]) & 3;
			    return acc_calc_d(f1+((inflg&4)>>1)+4*md, &bxi->m_x,*outb,*inb,inb[2],n);}

BX_SCALC(NAccFilt::sc_fn) {
	SCALC_BXI(NAccFilt); double *out = outb[0], *in = inb[0], *pv0 = &bxi->m_x, *pv1 = (double*)bxi->m_p0;
	int md = (int)lround(inb[1][0]) & 3, m = bxi->m_arg, flg1 = inflg&1;
	if (inflg&16) {
		...
		acc_calc_m(flg1+2+4*md, pv0, out, in, mul, n);
		for (int i=1; i<m; i++) { for (int j=0; j<n; j++) mul[j] *= mul2[j];
					  acc_calc_m(f43, pv1+i-1, out,out, mul,n); }
	} else {
		double spr = inb[4][0];
		if (spr<1e-14) {
			if (inflg&4) {
				...
				acc_calc_m(flg1 + 2 + 4*md, pv0, out, in, mul, n);
				for (int i=1,f43=3+4*md; i<m; i++) acc_calc_m(f43, pv1+i-1, out, out, mul, n);
			} else {
				double mul = att2mul(inb[2][0]);
				acc_calc_m(flg1 + 4*md, pv0, out, in, &mul, n);
				for (int i=1,f41=1+4*md; i<m; i++) acc_calc_m(f41, pv1+i-1, out, out, &mul, n);
		}} else {
			...
			if (inflg&4) {
				...
				for (int i=1,f41=1+4*md; i<m; i++) mul *= mul2,
					acc_calc_m(f41, pv1+i-1, out,out, &mul,n);
	}}}
	return 1;
}
...
//? dmp can be non-constant
struct AccFilt : BoxInst_B0 { static scf_t sc_ini, sc_f1;  double m_v; };

BX_SCALC(AccFilt::sc_ini) { SCALC_BXI(AccFilt); bxi->m_v = 0.0; CALC_FW(sc_f1); }
BX_SCALC(AccFilt::sc_f1) {
...
struct QHiFilt : BoxInst_B0 { static scf_t sc_ini, sc_f1;  double m_x1, m_y1; }; // m_arg: flg

BX_SCALC(QHiFilt::sc_ini) { SCALC_BXI(QHiFilt); bxi->m_arg = 4; bxi->m_x1 = bxi->m_y1 = 0.0; CALC_FW(sc_f1); }

#define QHC1(J) (z = pz[J], a = 1.0-z, b = a-z)
#define QHC2(J) (x = q[J], y1 = to[J] = b*y1 + a*(x-x1), x1 = x)
#define QHC0(J) (y1 = to[J] = b*y1)
BX_SCALC(QHiFilt::sc_f1) {
	SCALC_BXI(QHiFilt); double x,z,a,b, x1=bxi->m_x1, y1=bxi->m_y1, *to=outb[0], *q=inb[0], *pz=inb[1];
	switch((inflg|bxi->m_arg)&7) {
		case 4: if (fabs(*pz)<1e-99) return BOX_CP0; else bxi->m_arg &= ~4;
		case 0: QHC1(0); QHC2(0); x1 = *q; for (int i=1; i<n; i++) QHC0(i); break;
		case 5: if (fabs(*pz)<1e-99) return BOX_CP0; else bxi->m_arg &= ~4;
...
struct Q1pFilt : BoxInst_B0 { static scf_t sc_ini, sc_f1;  double m_y1; }; // m_arg: flg

BX_SCALC(Q1pFilt::sc_ini) { SCALC_BXI(Q1pFilt); bxi->m_arg = 16+4*bxi->m_arg; bxi->m_y1 = 0; CALC_FW(sc_f1); }

#define Q1PF(L) if (fabs(*pz)<1e-99) goto cp##L; else bxi->m_arg &= ~16
...
BX_SCALC(Q1pFilt::sc_f1) {
	SCALC_BXI(Q1pFilt); double x, a, b, y1 = bxi->m_y1, *to = outb[0], *q = inb[0], *pz = inb[1];
	switch((inflg|bxi->m_arg)&31) {
		...
		case 18: bxi->m_arg&=~16; case  2: Q1P0(1.0-fabs(a));
		case 19: bxi->m_arg&=~16; case  3: Q1P1(1.0-fabs(a));
		case 22: bxi->m_arg&=~16; case  6: Q1P0(1.0-   a   );
		case 23: bxi->m_arg&=~16; case  7: Q1P1(1.0-   a   );
		case 26: bxi->m_arg&=~16; case 10: Q1P0(1.0+   a   );
		case 27: bxi->m_arg&=~16; case 11: Q1P1(1.0+   a   );
...
struct FOAFilt : BoxInst_B0 { static scf_t sc_ini, sc_f1;  double m_x1, m_y1, m_z; };

BX_SCALC(FOAFilt::sc_ini)   {	SCALC_BXI(FOAFilt); double x = inb[1][0];
...
struct QLoHiFilt : BoxInst_B0 { static scf_t sc_ini, sc_f1;  double m_ed[2], m_yv[2]; }; // arg: flg (2:hi)

BX_SCALC(QLoHiFilt::sc_ini) { SCALC_BXI(QLoHiFilt); qlh_ini(bxi->m_ed, inb[1][0], inb[2][0]);
			      bxi->m_yv[0] = bxi->m_yv[1] = 0.0; CALC_FW(sc_f1); }

BX_SCALC(QLoHiFilt::sc_f1) { SCALC_BXI(QLoHiFilt); 
			     return qlh_calc(*outb,*inb, bxi->m_yv,bxi->m_ed, n, (inflg&1)+(bxi->m_arg&2)); }
...
//? ==> .!b.misc.map01
struct QLHSeqFilt : BoxInst_B1 { static scf_t sc_ini, sc_f1, sc_fn;   int m_nf, m_flg;  };

BX_SCALC(QLHSeqFilt::sc_ini) {
	SCALC_BXI(QLHSeqFilt); int nf = (int)lround(inb[3][0]), flg = 0;
	if (nf<0) nf = -nf, flg = 2;  if (nf>999) nf=999;
	bxi->m_nf = nf; bxi->m_flg = flg;
	double *edyv = (double*)(bxi->alloc0(32*nf)); memset(edyv,0,32*nf);
...
BX_SCALC(QLHSeqFilt::sc_f1) { SCALC_BXI(QLHSeqFilt); double *edyv = (double*)bxi->m_p0;
...
BX_SCALC(QLHSeqFilt::sc_fn) {
	...
	double *p = *inb, *q = *outb, *edyv = (double*)bxi->m_p0;
...
void b_filt_v_init(ANode *rn) {
	qmk_box(rn, "=qhi1", QMB_A_BX(QHiFilt), 0, 2, 33, "qh1", "i*r", "in$par", "uuu3%a");
	qmk_box(rn, "=q1p*", QMB_A_BX(Q1pFilt), 0, 2, 33, "q1p", "i*R*1", "in$a", "uuu3%a");
	qmk_box(rn, "=q1p+", QMB_A_BX(Q1pFilt), 1, 2, 33, "q1p", "1");
	qmk_box(rn, "=q1p-", QMB_A_BX(Q1pFilt), 2, 2, 33, "q1p", "1");
}

void b_filt_misc_init(ANode * rn) {
	qmk_box(rn, "=acc",   QMB_A_BX(AccFilt), 0, 2, 1, "accF", "i*r", "in$dmp", "uuu3%a");
	qmk_box(rn, "=accN",  QMB_A_BX(NAccFilt), 0, 5, 1, "nacF", "i*r", "in$md$dmp$rpt$sprd", "uuu3%a");
	qmk_box(rn, "=qlow",  QMB_A_BX(QLoHiFilt), 0, 3, 33, "qlh", "i*R*1", "in$fq$amp", "uuu3%a");
	qmk_box(rn, "=qhigh", QMB_A_BX(QLoHiFilt), 2, 3, 33, "qlh", "1");
	qmk_box(rn, "=qlh*",  QMB_A_BX(QLHSeqFilt), 0, 6, 33, "qlh*", "1i*", "in$fq0$fq1$n$scl$amp");
	qmk_box(rn, "=f/dly", QMB_A_BX(FOAFilt), 0, 2, 33, "ffoa", "1i*", "in$dly");
}

void b_filt_echo_init(ANode * rn) {
	char nm[8]; memcpy(nm, "=echo01", 8); 
	qmb_arg_t qa = QMB_A_BX(SparseRF);
