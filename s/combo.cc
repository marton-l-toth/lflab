...
void ComboBoxModel::set_size() { 
	int nb = n_bx, siz = 8*nb + sizeof(ComboBoxInst) + ((niof&31) ? sizeof(FeedbackBoxInst) : 0);
	BoxModel **pm = boxm; for (int i=0; i<nb; i++) siz += pm[i]->size();  m_size = siz;   }
